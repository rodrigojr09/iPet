...
export default function Navbar() {
	const { account, profile,logout,status } = useAuth();
				...
				<div className="flex items-center space-x-4">
					{profile ? (
						<>
							...
							<button className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm transition-colors" onClick={logout}>
								Sair
							</button>
						</>
					) : (
