...
export default async function handler(
	...
	form.parse(req, async (err, fields, files) => {
		if (err) {
			console.error("Erro ao analisar o formulario:", err);
		...
		if (!authorId) {
			return res.status(400).json({ error: "ID do autor e obrigatorio" });
		}
		...
		if (imagem) {
			try {
				...
				console.error("Erro ao processar imagem:", uploadErr);
				return res
					.status(500)
